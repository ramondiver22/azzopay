              ...
              <form role="form" action="{{route('submitregister')}}" method="post">
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                          <div class="input-group">
                            ...
                            <input class="form-control" placeholder="{{$lang["regiter_mobile"]}}" type="number" name="phone" required>
                          </div>
                          @if ($errors->has('phone'))
                ...
                <div class="custom-control custom-control-alternative custom-checkbox">
                  ...
                  <label class="custom-control-label" for=" customCheckLogin">
                    <span class="text-muted">{{$lang["regiter_agree_to"]}} <a href="{{route('terms')}}">{{$lang["regiter_terms_and_conditions"]}}</a> @if($set->stripe_connect==1) {{$lang["regiter_and_the"]}} <a href="https://stripe.com/connect-account/legal/full">{{$lang["regiter_stripe_connected_account_agreement"]}}</a>@endif</span>
                  </label>
                </div>
                @if($set->recaptcha==1)
                  {!! app('captcha')->display() !!}
                ...
                <div class="text-center">
                  <button type="submit" class="btn btn-neutral btn-block my-4 text-uppercase"  id="update_password">{{$lang["regiter_create_account"]}}</button>
                  <div class="loginSignUpSeparator"><span class="textInSeparator">{{$lang["regiter_or"]}}</span></div>
                  <a href="{{route('login')}}" class="btn btn-primary btn-block my-0 text-uppercase">{{$lang["regiter_got_an_account"]}}</a>
                </div>
              </form>
            </div>
          </div>
