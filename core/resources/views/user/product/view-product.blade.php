					...
					<div class="tt-product-single-info">
						<div class="tt-add-info">
                            ...
                            @if($product->quantity!=0)
                                <li><span>{{$lang["product_view_product_availability"]}}:</span> {{$product->quantity}} {{$lang["product_view_product_in_stock"]}}</li>
                            @else
                                <li><span>{{$lang["product_view_product_availability"]}}:</span> {{$lang["product_view_product_out_of_stock"]}}</li>
                            @endif
							</ul>
						</div>
                        ...
                        <form action="{{route('update.cart')}}" method="post">
                            <div class="tt-wrapper">
                                <div class="tt-row-custom-01">
                                    ...
                                    <div class="col-item">
                                        @if($product->quantity!=0)
                                        <button type="submit" class="btn btn-lg"><i class="icon-f-39"></i>{{$lang["product_view_product_add_to_chart"]}}</button>
                                        @else
                                            <a href="#"  class="btn btn-lg disabled"><i class="icon-f-39"></i>{{$lang["product_view_product_out_of_stock"]}}</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
