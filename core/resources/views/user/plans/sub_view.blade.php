          ...
          <div class="card card-profile bg-white border-0 mb-5">
            <div class="card-body pt-7 px-5">
              ...
              <form role="form" action="{{ route('submit.plancharge')}}" method="post" id="modal-details">
                ...
                <div class="form-group row">
                    <div class="col-lg-12">
                      <select class="form-control select" name="status" required>
                          <option value="">{{$lang["plans_auto_renewal"]}}</option>
                          <option value="1">{{$lang["plans_yes"]}}</option>
                          <option value="0">{{$lang["plans_no"]}}</option>
                      </select>
                    </div>
                </div>
                @endif
                <div class="text-center">  
                  <small>{{$link->intervals}} {{$lang["plans_renewal"]}}</small>
                </div>
                          ...
                          <div class="card bg-white border-0 mb-0">
                            ...
                            <div class="card-header" id="headingTwo">
                                <div class="text-left" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="fadse" aria-controls="collapseTwo">
                                  <h4 class="mb-0 font-weight-bolder">{{$lang["plans_pay_with_account"]}}</h4>
                                </div>
                            </div>
                              ...
                              <div class="card-body text-center">
                                @if (Auth::guard('user')->check())
                                  <form method="post" role="form" action="{{route('submit.plancharge')}}">
                                    @csrf
                                    <h4 class="mb-0">{{$lang["plans_account_balance"]}}</h4>
                                    <h1 class="mb-1 text-muted">{{$currency->symbol.number_format($user->balance)}}</h1>
                                    <div class="text-center">
                                      <button type="submit" class="btn btn-neutral btn-block" form="modal-details">{{$lang["plans_pay_now"]}}</button>
                                    </div>
                                  </form>
                                @else
                                  @php Session::put('oldLink', url()->current()); @endphp
                                  <h3 class="mb-3 text-muted">{{$lang["plans_login_to_make_payment"]}}</h3>
                                  <a href="{{route('login')}}" class="btn btn-neutral btn-block">{{$lang["plans_login"]}}</a>
                                @endif
                              </div>
                            </div>
                          </div>
                ...
                <div class="text-center">
                  <button type="button" data-toggle="modal" data-target="#fund" class="btn btn-neutral btn-block my-4"><i class="fad fa-external-link"></i> {{$lang["plans_subscribe"]}}</button>
                </div>
