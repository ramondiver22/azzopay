...
<html class="no-js" lang="en">
	...
	<nav class="panel-menu mobile-main-menu">
		<div class="mm-navbtn-names">
			<div class="mm-closebtn">{{$lang["shop_layout_close"]}}</div>
			<div class="mm-backbtn">{{$lang["shop_layout_back"]}}</div>
		</div>
	</nav>
	<!-- tt-mobile-header -->
					...
					<div class="tt-desctop-parent-menu tt-parent-box">
						<div class="tt-desctop-menu">
							<nav>
								<ul>
                                    <li class="dropdown">
										<a href="{{route('user.dashboard')}}">{{$lang["shop_layout_set_up_your_store_for_free"]}}</a>
									</li>
								</ul>
							</nav>
						</div>
						...
						<div class="tt-cart tt-dropdown-obj" data-tooltip="Cart" data-tposition="bottom">
							...
							<div class="tt-dropdown-menu">
								<div class="tt-mobile-add">
									<h6 class="tt-title">{{$lang["shop_layout_shopping_cart"]}}</h6>
									<button class="tt-close">{{$lang["shop_layout_close"]}}</button>
								</div>
									...
									<div class="tt-cart-layout">
										...
										<div class="tt-cart-content">
											...
											<div class="tt-cart-total-row">
												<div class="tt-cart-total-title">{{$lang["shop_layout_subtotal"]}}:</div>
												<div class="tt-cart-total-price">{{$currency->symbol.$gtotal}}</div>
											</div>
											<div class="tt-cart-btn">
												<div class="tt-item">
													@if(count($cart)>0)
														<a href="{{route('user.sask', ['id'=>$val->uniqueid])}}" class="btn">{{$lang["shop_layout_proceed_to_checkout"]}}</a>
													@endif
												</div>
											</div>
										</div>
...
<div class="tt-breadcrumb">
	<div class="container">
		<ul>
            <li><a href="{{route('store.link', ['id' => $store->store_url])}}">{{$lang["shop_layout_home"]}}</a></li>
				...
				<div class="col-md-6 col-lg-6 col-xl-6">
					<div class="tt-mobile-collapse">
						...
						<div class="tt-collapse-content">
                            <address>
                                <span>{{$lang["shop_layout_by"]}} {{$merchant->business_name}}</span>
				...
				<div class="col-md-6 col-lg-6 col-xl-6 text-right">
					<div class="tt-newsletter">
						<div class="tt-mobile-collapse">
							<div class="tt-collapse-content">
								<address>
									<span>{{$lang["shop_layout_phone"]}}: {{$merchant->phone}}</span>
									<span>{{$lang["shop_layout_email"]}}: <a href="mailto:{{$merchant->support_email}}">{{$merchant->support_email}}</a></span>
								</address>
							</div>
						</div>
					</div>
