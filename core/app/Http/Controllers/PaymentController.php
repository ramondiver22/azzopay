...
class PaymentController extends Controller {
    ...
    public function genInstallmentsTable() {
        try {
            
            $amount = grava_money(request("amount"));
            $userId = grava_money(request("user"));

            $installments = Array();

            if ($amount > 0) {
                $installments = CreditcardFeeTable::getUserInstallmentsTable($amount, $userId);
            }

            ob_start();
            foreach($installments as $installment) {
                ?>
                <option value="<?php echo $installment['installment'] ?>">
                    <?php echo $installment["installment"] . "x de " . number_format($installment["installmentValue"], 2, ",", "."); ?>
                </option>
                <?php
            }
            ...
            $success = Array("success" => true, "html" => $options);
